...
{
    *v = ici_fetch(objwsupof(o)->o_super, k);
    return *v == NULL;
}
...
{
    return ici_fetch(objwsupof(o)->o_super, k);
}
...
channel_new(ici_objwsup_t *klass)
{
    ...
    ICI_OBJ_SET_TFNZ(chan, channel_tcode, O_SUPER, 1, 0);
    chan->o_head.o_super = channel_class;
    chan->c_capacity = capacity;
    chan->c_altobj = NULL;
    ici_rego(chan);
    return ici_ret_with_decref(objof(chan));
}
...
channel_get(ici_objwsup_t *inst)
{
    ...
    if (ici_method_check(objof(inst), channel_tcode))
        return 1;
    q = channelof(inst)->c_q;
    while (ici_array_nels(q) < 1)
    {
        if (ici_waitfor(objof(q)))
            return 1;
    }
    o = ici_array_rpop(q);
    ici_wakeup(objof(q));
...
channel_alt(ici_objwsup_t *ign)
{
    ...
    if (ici_typecheck("a", &alts))
        return 1;
    if (alt_setup(alts, objof(alts)))
	return 1;
    while ((idx = alt(alts)) == -1)
    {
        if (ici_waitfor(objof(alts)))
            return 1;
    }
    alt_setup(alts, NULL);
...
{
    {CF_OBJ, "new", channel_new},
    {CF_OBJ, "get", channel_get},
    {CF_OBJ, "put", channel_put},
    {CF_OBJ, "capacity", channel_capacity},
    {CF_OBJ, "queued", channel_queued},
    {CF_OBJ, "alt", channel_alt},
    {CF_OBJ}
};
